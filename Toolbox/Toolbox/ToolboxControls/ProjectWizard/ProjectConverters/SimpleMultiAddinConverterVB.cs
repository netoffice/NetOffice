...
{
    internal class SimpleMultiAddinConverterVB : Converter
    {        
        #region Fields
        ...
        internal SimpleMultiAddinConverterVB(ProjectOptions options) : base(options)
        { 
        ...
        private void ReplaceMarker()
        {
            ...
            _addinFile = _addinFile.Replace("$applicationField$", String.Format("\tPrivate _application As COMObject", Options.OfficeApps[0]));
            ...
            {
                registerCode += ReadProjectTemplateFile("SimpleMultiAddinVB.Register.txt").Replace("%AppName%", item).Replace("%HiveKey%", Options.HiveKey).Replace("%Name%", Options.AssemblyName).Replace("%Description%", Options.AssemblyDescription).Replace("%LoadBehavior%", Options.LoadBehaviour.ToString()).Replace("%officAddinKey%", Options.RegistryKeys[i] + "\\" + Options.AssemblyName + ".Addin");
                unregisterCode += ReadProjectTemplateFile("SimpleMultiAddinVB.UnRegister.txt").Replace("%AppName%", item).Replace("%HiveKey%", Options.HiveKey).Replace("%officAddinKey%", Options.RegistryKeys[i] + "\\" + Options.AssemblyName + ".Addin");
                i++;
            }
            ...
            _addinFile = _addinFile.Replace("$applicationConstruction$", String.Format("\t\t\t_application = Core.Default.CreateObjectFromComProxy(Nothing, application)", Options.OfficeApps[0]));
            ...
            if (Options.UseRibbonUI)
            {
                ...
                _addinFile = _addinFile.Replace("$ribbonImplement$", ReadProjectTemplateFile("SimpleMultiAddinVB.RibbonImplement.txt"));
            ...
            if (Options.UseTaskPane)
            {
                ...
                _addinFile = _addinFile.Replace("$taskpaneImplement$", ReadProjectTemplateFile("SimpleMultiAddinVB.TaskPaneImplement.txt").Replace("$safeprojectname$", Options.AssemblyName));
        ...
        private void ReadRessourceFiles()
        {
            ...
            _projectFile = ReadProjectTemplateFile("SimpleMultiAddinVB.Project.txt");
            _projectUserFile = ReadProjectTemplateFile("SimpleMultiAddinVB.Project_User.txt");
            _addinFile = ReadProjectTemplateFile("SimpleMultiAddinVB.Addin.txt");
            _assemblyFile = ReadProjectTemplateFile("SimpleMultiAddinVB.AssemblyInfo.txt");
        }
